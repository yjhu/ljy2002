...
void show_pole(const char pole, int pole_content[], const int x_pos)
{
    for (int layer = 0; layer < hanoi_layers; layer++) {
        cct_gotoxy(x_pos, hanoi_layers - layer - 1);
        if (pole_content[layer] > 0)
            cout << pole_content[layer];
        else
            cout << ' ';
    }
    cct_gotoxy(x_pos, hanoi_layers);
    cout << pole;
}
...
/***************************************************************************
  ...
  说    明：1、函数名、形参、返回类型均不准动
            2、本函数不允许出现任何形式的循环
***************************************************************************/
void hanoi(int n, char src, char tmp, char dst, bool show_content = true, int delay = 5)
{
    ...
    {
        hanoi(n - 1, src, tmp, tmp, show_content, delay);
        ...
        i++;
        hanoi(n - 1, tmp, tmp, dst, show_content, delay);
...
int main()
{
    ...
    hanoi(0, src, tmp, dst, whether, speed);
    hanoi(n, src, tmp, dst, whether, speed);
